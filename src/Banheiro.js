...
// import chuveiro from './assets/chuveiro.png'; // Exemplo de ícone para o chuveiro

export default function BanheiroSuite() {
    ...
    useEffect(() => {
        const buscarDados = async () => {
            try {
                ...
                setAmbiente(data.residencia.banheiro_suite);
            } catch (error) {
    ...
    if (!ambiente || !ambiente.dispositivos) {
        return <div style={fundo}>Nenhum dispositivo encontrado no banheiro.</div>;
    }
                ...
                <section style={cardContainer}>
                    {ambiente.dispositivos.map(dispositivo => (
                        <div key={dispositivo.id} style={cardBase}>
                            ...
                            {dispositivo.tipo === 'chuveiro_eletrico' && (
                                <>
                                    {/* <img src={chuveiro} style={imgCss} alt="Chuveiro Elétrico" /> */}
                                    <p style={txtcards}>Temperatura: {dispositivo.estado.temperatura}</p>
                                    <p style={txtcards}>Status: {dispositivo.estado.ligado ? 'Ligado' : 'Desligado'}</p>
                                    <p style={txtcards}>Consumo: {dispositivo.estado.consumoWatts} W</p>
                                </>
                            )}
